...
export default function PlayerHomePage() {
  ...
  const xpPct =
    nextLvXp > prevLvXp
      ? Math.min(
          100,
          Math.round(((currentXp - prevLvXp) / (nextLvXp - prevLvXp)) * 100)
        )
      : 0;

  const donePct = dailyLog?.summary?.completion_rate ?? 0;
  const rewardUnlocked =
    dailyLog?.summary?.reward_78_unlocked ||
    dailyLog?.summary?.reward_100_unlocked;

  const tasks = dailyLog?.tasks
    ? Object.entries(dailyLog.tasks).map(([id, data]) => ({
        id,
        ...data,
      }))
    : [];

  if (logLoading || sessionLoading) {
    return (
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <span
              className={`text-sm font-bold ${
                donePct >= 78 ? 'text-green-400' : 'text-amber-400'
              }`}
            >
              {donePct}%
            </span>
          ...
          <div className="h-2.5 bg-slate-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-700 ${
                donePct >= 78 ? 'bg-green-500' : 'bg-amber-500'
              }`}
              style={{ width: `${donePct}%` }}
            />
          </div>
        ...
        {tasks.length === 0 ? (
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 text-center">
            ...
            <p className="text-slate-600 text-xs mt-1">
              Nếu Firebase đã có task, hãy kiểm tra Firebase Rules hoặc
              permission_denied.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-yellow-500 text-xs font-medium mt-0.5">
                      +{task.xp_earned ?? task.xp_reward ?? 0} XP
                    </p>
