...
## 8. PHASE 2 — TASK / REWARD / SHOP EXPANSION BOUNDARY

Phase 2 expands task templates, reward catalog, reward batches, and shop/store items.

This is a DATA EXPANSION phase, not an economy engine rewrite.

The following rules remain locked:

- Do not change delta-based XP/Points granting.
- Do not change `xp_granted` or `points_granted` ledger logic.
- Do not add task-level processed flags.
- Do not use `rewardProcessed`, `claimed`, or task ordering hacks.
- Do not let reward/shop flows modify `xp_granted` or `points_granted`.
- Do not let UI directly grant XP or Points.
- Do not let shop purchase directly corrupt daily summary ledger.

### 8.1. Task Expansion

Adding more `task_templates` or `task_batches` is allowed if:

- XP and point rewards are integers.
- XP/point ratio follows the locked economy rules.
- Daily reset still selects/clones tasks according to Coregame.
- Task approval remains the only path to XP/Points.

### 8.2. Reward Catalog Expansion

Adding more `rewards` and `reward_batches` is allowed if:

- Reward unlock logic remains based on locked completion gates.
- Reward metadata does not become a new economy ledger.
- Rewards do not directly modify `xp_granted` or `points_granted`.

### 8.3. Shop Expansion

Adding more `store_items` is allowed if:

- Shop reads `users/{user_id}/stats/total_points`.
- Shop may deduct `total_points` only through an approved transaction/backend-safe flow.
- Shop must not modify daily summary ledger.
- Shop must not modify `xp_granted` or `points_granted`.
- Shop transaction history must remain separate from daily task reward ledger.

### 8.4. When This Lock Must Be Revised

Revise this `ENGINE_RULES_LOCK.md` only if Phase 2 introduces:

- New XP/Points formulas.
- New completion reward gates beyond 78% or 100%.
- New streak bonus rules.
- New level formula or level threshold behavior.
- New task approval states.
- New shop payment mechanics that affect `total_points`.
- New reward claiming mechanics that affect player stats.
